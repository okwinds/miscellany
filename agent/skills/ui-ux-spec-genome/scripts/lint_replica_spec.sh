...
err_count=0
warn_count=0
tmp_empty_templates_out=""
cleanup() {
  if [[ -n "${tmp_empty_templates_out}" && -f "${tmp_empty_templates_out}" ]]; then
...
run_check() {
  ...
  local out
  out="$(rg -n --pcre2 -S "${rg_globs[@]}" "${rg_args[@]}" "$root" || true)"
  if [[ -n "$out" ]]; then
...
if [[ $strict -eq 1 ]]; then
  ...
  if ! SPEC_ROOT="$root" INCLUDE_ASSETS="$include_assets" python_check_empty_templates >"${tmp_empty_templates_out}" 2>/dev/null; then
